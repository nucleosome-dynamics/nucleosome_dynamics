...
calls <- GRanges(tab$space, IRanges(tab$start, tab$end))
calls <- keepSeqlevels(calls, seqlevelsInUse(calls))

## Do it ######################################################################

message("-- looking for Nucleosome Free regions")
nfr <- rd2df(irLs2gr(lapply(
    ranges(split(calls, seqnames(calls))),
    function(x) {
